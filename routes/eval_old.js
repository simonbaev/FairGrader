		...
		if(!req.params.term || !/^20[12]\d0[257]$/.test(req.params.term)) {
			...
			.exec(function(err, projectData) {
				...
				if(projectData.length === 1) {
					res.redirect(req.baseUrl + '/' + projectData[0].term + '/' + projectData[0].course.key + '/' + projectData[0].project.key);
				}
				else {
					let dataForRender = {
						type: 'data',
					...
					if(dataForRender.data.length === 1) {
						...
						res.render('eval_old', dataForRender);
					}
				}
			});
		}
			...
			if(!req.params.course || !/^[a-zA-Z]{2,5}_\d{2,5}$/i.test(req.params.course)) {
				...
				.exec(function(err, projectData) {
					...
					if(projectData.length === 1) {
						...
						if(dataForRender.data.length === 1) {
							console.log('1');
							res.redirect(dataForRender.urlPrefix + '/' + dataForRender.data[0].key);
						}
						else {
							res.render('eval_old', dataForRender);
						}
