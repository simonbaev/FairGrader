...
exports.setup = function(next) {
	console.log('Adding report records...');
	Project.findOne({
		term: defaultTerm,
	...
	.exec(function(err, projectItem){
		...
		.forEach(function(entry, index, array) {
			entry.members.forEach(function(member){
				...
				fnReportArray.push(report.save.bind(report));
			});
		});
		async.parallel(fnReportArray,function(err,reports){
			if(err) {
				return next(err);
			}
			next(null, 'Reports (' + reports.length + ')');
		});
	});
};
